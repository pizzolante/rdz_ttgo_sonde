...
bool ConnTelegram::sendTelegramMessage(const char *message) {
	...
	while (client.available() == 0) {
		if (millis() - timeout > 10000) {
			LOG_E(TAG, "Telegram API timeout");
			client.stop();
			return false;
		}
		delay(10);
